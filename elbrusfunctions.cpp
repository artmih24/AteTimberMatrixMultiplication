...
void MulKernel48Bias(float *inp, float *wts, float *bias, float *out, int L, int F) {
    // timespec start, end, tdiff;
    // clock_gettime(CLOCK_THREAD_CPUTIME_ID, &start);
    ...
    sum11 = hadds4bias(partsum11, partsum12, partsum13, partsum14, bias1);
    sum12 = hadds4bias(partsum15, partsum16, partsum17, partsum18, bias2);

    sum21 = hadds4bias(partsum21, partsum22, partsum23, partsum24, bias1);
    sum22 = hadds4bias(partsum25, partsum26, partsum27, partsum28, bias2);

    sum31 = hadds4bias(partsum31, partsum32, partsum33, partsum34, bias1);
    sum32 = hadds4bias(partsum35, partsum36, partsum37, partsum38, bias2);

    sum41 = hadds4bias(partsum41, partsum42, partsum43, partsum44, bias1);
    sum42 = hadds4bias(partsum45, partsum46, partsum47, partsum48, bias2);

    sum51 = hadds4bias(partsum51, partsum52, partsum53, partsum54, bias1);
    sum52 = hadds4bias(partsum55, partsum56, partsum57, partsum58, bias2);

    sum61 = hadds4bias(partsum61, partsum62, partsum63, partsum64, bias1);
    sum62 = hadds4bias(partsum65, partsum66, partsum67, partsum68, bias2);
...
void MulKernel48Relu(float *inp, float *wts, float *bias, float *out, int L, int F) {
    // timespec start, end, tdiff;
    // clock_gettime(CLOCK_THREAD_CPUTIME_ID, &start);
...
int ElbrusMatrixMulBias(float *inp, float *wts, float *bias, float *out, long Q, long L, long F) {
    ...
    if (Q >= F) {
        ...
        #pragma omp parallel for
        {
            for (long q = 0; q < Q; q += 6)
                for (long f = 0; f < F; f += 8)
                    MulKernel48Bias(inp + q * L, wts + f * L, bias + f, out + f + q * F, L, F);
        }
        clock_gettime(CLOCK_THREAD_CPUTIME_ID, &end);
    }
    else {
